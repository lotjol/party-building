define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'cms-base', 'cms-clipboard'], function ($, undefined, Backend, Table, Form, Base, ClipboardJS) {
    ...
    var Controller = {
        ...
        config: function (form) {
            ...
            form.on("click", ".btn-command", function () {
                if ($(".config-list a.active").length == 0) {
                    Layer.msg("请选择")
                    return false;
                }
        ...
        arclist: function (form) {
            ...
            form.on("click", ".btn-command", function () {
                ...
                if (field.find("option[value!='']:selected").length > 0) {
                    $("option" + (field.find("option:selected").length ? ":selected" : "") + "[value!='']", field).each(function (i, j) {
                        columns.push("{$" + data.id + "." + $(j).attr("value") + "}");
                    });
                }
                if (addon.find("option[value='']:selected").length == 0) {
                    $(addon.find("option" + (addon.find("option[value='true']:selected").length > 0 ? "[value!=''][value!='true']" : ":selected"))).each(function (i, j) {
                        columns.push("{$" + data.id + "." + $(j).attr("value") + "}");
                    });
                }
                if (channelfield.find("option:selected").length > 0) {
                    $(channelfield.find("option:selected")).each(function (i, j) {
                        columns.push("{$" + data.id + ".channel." + $(j).attr("value") + "}");
                    });
                }
                if (userfield.find("option:selected").length > 0) {
            ...
            form.on("change", "#channel,#model", function () {
                ...
                if (currentId[name]) {
                    ...
                    $("optgroup", orderby).remove();
                    addon.html('<option value="">无</option>');
                    addon.data("id", currentId[name]);
                    refresh(addon);
                    refresh(orderby);
                }
            ...
            form.on("change", "#orderby", function () {
                var value = $(this).val();
                if (value && $("#addon option[value='" + value + "']:selected").length === 0) {
                    addon.find("option[value='" + value + "']").prop("selected", true);
                    refresh(addon);
                }
            });
        },
