...
class Service
{
    ...
    public static function autolinks($content)
    {
        ...
        $autolinkArr = [];
        $tagList = Tag::where('autolink', 1)->cache(true, null, 'cms')->where('status', 'normal')->select();
        foreach ($tagList as $index => $item) {
            $autolinkArr[$item['name']] = ['text' => $item['name'], 'type' => 'tag', 'url' => $item['fullurl']];
        }
        $autolinkList = Autolink::where('status', 'normal')->cache(true, null, 'cms')->order('weigh DESC,id DESC')->select();
    ...
    {
        $config = get_addon_config('cms');
        $type = $type ?: $config['auditnotice'];
        $template_id = $template_id ?: $config['noticetemplateid'];

        try {
            if ($type == 'email') {
                //邮件通知
                if (Validate::is($template_id, 'email')) {
                    $email = new Email;
                    $email->to($template_id)->subject($content)->message('<div style="min-height:550px; padding: 100px 55px 200px;">' . $content . '</div>')->send();
                }
            } elseif ($type == 'dinghorn') {
                //钉钉通知插件(dinghorn)
    ...
    public static function getFilterList($source, $source_id, $filter, $params = [], $multiple = false)
    {
        ...
        if (in_array($source, ['model', 'diyform'])) {
            ...
            if (isset($setting['filterfields'])) {
                foreach ($setting['filterfields'] as $index => $name) {
                    $title = $setting['titlelist'][$name] ?? $name;
                    $filterlist = $setting['filterlist'][$name] ?? '';
        ...
        foreach ($fieldsList as $k => $field) {
            ...
            $content = isset($field['filter_list']) && $field['filter_list'] ? $field['filter_list'] : $field['content_list'];
            if (!$content) {
                continue;
            }
            //多选值字段需要做特殊处理
            if (in_array($field['type'], ['selects', 'checkbox', 'array', 'selectpages'])) {
                $multiValueFields[] = $field['name'];
            }
            $success = 0;
            $field['favisible'] = is_array($field['favisible']) ? $field['favisible'] : [];
            foreach ($field['favisible'] as $subindex => $subitem) {
                if (!isset($params[$subitem['field']])) {
                    break;
                }
                $visibleFields[$subitem['field']] = $visibleFields[$subitem['field']] ?? [];
                $visibleFields[$subitem['field']][] = $field['name'];
                switch ($subitem['operate']) {
                    case "=":
                    case "==":
                        if ($params[$subitem['field']] == $subitem['value']) {
                            $success++;
                        }
                        break;
                    case ">":
                    ...
                    case "<=":
                        ...
                        break;
                    case "in":
                        if ($params[$subitem['field']] && stripos($params[$subitem['field']], $subitem['value']) !== false) {
                            $success++;
                        }
                        break;
                    case "regex":
                        if ($params[$subitem['field']] && preg_match("/{$subitem['value']}/", $params[$subitem['field']])) {
                            $success++;
                        }
                        break;
                    default:
            ...
            //如果不匹配则移除验证
            if ($success != count($field['favisible'])) {
                continue;
            }
            $fields[] = [
                'name'    => $field['name'],
        ...
        foreach ($fields as $k => $v) {
            ...
            foreach ($all as $m => $n) {
                ...
                $active = ($m === '' && !$filterArr) || ($m !== '' && in_array($m, $filterArr));
                if ($active) {
                ...
                $currentParams = array_merge($prepare, array_intersect_key($params, array_flip(['orderby', 'orderway', 'multiple', 'q'])));
                //移除动态显示的子选项参数
                $currentParams = array_diff_key($currentParams, array_flip($visibleFields[$v['name']] ?? []));
                $url = '?' . str_replace(['%2C', '%3B'], [',', ';'], http_build_query($currentParams));
    ...
    public static function appendTextAndList($source, $source_id, &$row, $isMultiArray = false)
    {
        ...
        $appendFunc = function ($field, $content, &$row) use ($fieldsType) {
            ...
            if (isset($row[$field])) {
                if (isset($content[$fieldValue])) {
                    $list = [$fieldValue => $content[$fieldValue]];
                } else {
                    $keys = $values = explode(',', $fieldValue);
                    foreach ($values as $index => &$item) {
                        $item = $content[$item] ?? $item;
                    }
                    $list = array_combine($keys, $values);
            ...
            $list = array_filter($list);
            $row[$field . '_text'] = $fieldType == 'array' ? $fieldValue : implode(',', $list);
            $row[$field . '_list'] = $fieldType == 'array' ? (array)json_decode($fieldValue, true) : $list;
        };
    ...
    {
        $fieldInfo = Fields::where(['source' => $source, 'source_id' => $source_id, 'name' => $field])->cache(true, null, 'cms')->find();
        if (!$fieldInfo) {
            return '';
        }
     ...
     * 判断定时发布文档
     */
    public static function checkPublishArchives()
    {
        //判断非Ajax请求
        if (!request()->isAjax()) {
            $time = time();
            //修改文档状态
            $result = Db::name("cms_archives")->where('status', 'prepare')->where('publishtime', '<', $time)->update(['status' => 'normal']);
            if ($result) {
                //清理文档缓存
                Cache::clear('cms');
            }
    ...
    public static function isInternalRequest()
    {
        ...
        if ($referer && isset($referer['host'])) {
            return $referer['host'] === $currentDomain;
        }
