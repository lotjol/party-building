...
-- 2.0.0 --
ALTER TABLE `__PREFIX__cms_archives` CHANGE `status` `status` ENUM('normal','hidden','draft','prepare','rejected','pulloff')  CHARACTER SET utf8mb4  COLLATE utf8mb4_general_ci  NOT NULL  DEFAULT 'normal'  COMMENT '状态';
ALTER TABLE `__PREFIX__cms_fields` ADD COLUMN  `favisible` varchar(1500) DEFAULT '' COMMENT '显示条件' AFTER `setting`;

-- 2.2.0 --
ALTER TABLE `__PREFIX__cms_archives` ADD `createby` VARCHAR(30)  NULL  DEFAULT ''  COMMENT '创建者'  AFTER `deletetime`;