...
class Channel extends Model
{
    ...
    public function getHasChildAttr($value, $data)
    {
        ...
        if (is_null(self::$parentIds)) {
            self::$parentIds = self::where('parent_id', '>', 0)->cache(true, null, 'cms')->where('status', 'normal')->column('parent_id');
        }
    ...
    public function getHasNavChildAttr($value, $data)
    {
        ...
        if (is_null(self::$navParentIds)) {
            self::$navParentIds = self::where('parent_id', '>', 0)->cache(true, null, 'cms')->where('status', 'normal')->where('isnav', 1)->column('parent_id');
        }
    ...
    public static function getContributeInfo($archives, $model = null)
    {
        ...
        foreach ($channelList as $k => $v) {
            if ($v['type'] == 'link' || (($archives || input('model_id')) && $model && $model['id'] != $v['model_id']) || (!$v['iscontribute']) || (!($config['channelcontribute'] ?? true) && $v['type'] == 'channel' ? true : false)) {
                $disabledIds[] = $v['id'];
            }
    ...
    public static function getChannelList($params)
    {
        ...
        if ($paginate) {
            ...
            $list = $channelModel->limit($limit)->cache($cacheKey, $cacheExpire, 'cms')->select();
        }
    ...
    public static function render(&$list, $imgwidth, $imgheight)
    {
        ...
        foreach ($list as $k => &$v) {
            $v['textlink'] = '<a href="' . $v['url'] . '">' . htmlentities($v['name']) . '</a>';
            $v['channellink'] = '<a href="' . $v['url'] . '">' . htmlentities($v['name']) . '</a>';
            $v['outlink'] = htmlentities($v['outlink']);
    ...
    public static function getNav($channel, $params = [])
    {
        ...
        if ($result === false) {
            $channelList = Channel::where($condition)
                ->where('status', 'normal')
                ->order('weigh desc,id desc')
                ->cache($cacheKey, $cacheExpire, 'cms')
                ->select();
            $tree = \fast\Tree::instance();
            $tree->init(collection($channelList)->toArray(), 'parent_id');
