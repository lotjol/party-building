...
class Comment extends Model
{
    ...
    {
        $comment = self::get($id);
        if ($comment && $comment['status'] === 'normal' && $comment['pid']) {
            $archives = $comment['type'] == 'archives' ? Archives::get($comment['aid']) : ($comment['type'] == 'special' ? Special::get($comment['aid']) : Page::get($comment['aid']));
            $parentComment = self::get($comment['pid'], 'user');
            if ($parentComment && $parentComment['subscribe'] && Validate::is($parentComment->user->email, 'email')) {
                ...
                $config = get_addon_config('cms');
                $archivesurl = request()->domain() . $archives['url'];

                $search = ['{sitename}', '{title}', '{comment_nickname}', '{comment_date}', '{comment_content}', '{url}', '{unsubscribe_url}'];
                $replace = [$config['sitename'], $archives['title'], $parentComment->user->nickname, datetime($parentComment['createtime']), $parentComment['content'], $archivesurl, $unsubscribe_url];

                $title = str_replace($search, $replace, $config['newreplytitletpl'] ?? '');
                $content = str_replace($search, $replace, $config['newreplycontenttpl'] ?? '');
                try {
                    $email = new Email;
                    $result = $email->to($parentComment->user->email)->subject($title)->message('<div style="min-height:550px; padding: 100px 55px 200px;">' . $content . '</div>')->send();
    ...
    public function source()
    {
        ...
        $model = $modelArr[$type] ?? $modelArr['archives'];
