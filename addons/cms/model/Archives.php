...
/**
 * 文章模型
 * @method static mixed getByDiyname($str) 通过自定义名称查询
 */
class Archives extends Model
{
    ...
    {
        return (bool)$this->getData("image");
    }
    ...
    public function getIspaidAttr($value, &$data)
    {
        ...
        $isvip = $channel && isset($channel['vip']) && $channel['vip'] && Auth::instance()->vip >= $channel->vip;
    ...
    {
        $titleArr = self::$config['downloadtype'] ?? [];
        ...
        foreach ($downloadurl as $index => $item) {
            if (!is_array($item)) {
                $urlArr = explode(' ', $item);
                $result['name'] = $index;
                $result['title'] = $titleArr[$index] ?? '其它';
                $result['url'] = cdnurl($urlArr[0], true);
                $result['password'] = $urlArr[1] ?? '';
                $list[] = $result;
            } elseif (isset($item['name']) && isset($item['url']) && $item['url']) {
                $item['url'] = cdnurl($item['url'], true);
                $result = $item;
                $result['title'] = $titleArr[$item['name']] ?? '其它';
    ...
    public function getTaglistAttr($value, &$data)
    {
        ...
        foreach ($tags as $k => $v) {
            ...
            $vars = [':name' => $v, ':diyname' => $v, ':id' => $tagList[$v_lower] ?? '0', ':year' => date("Y", $time), ':month' => date("m", $time), ':day' => date("d", $time)];
    ...
    public static function getArchivesList($params)
    {
        ...
        if ($channel !== '') {
            if ($type === 'son') {
                ...
                //所有子级
                $where['channel_id'] = ['in', Channel::getChannelChildrenIds($channel, false)];
            } else {
                $where['channel_id'] = ['in', $channel];
            }
        }
        //如果有设置标志,则拆分标志信息并构造condition条件
        if ($flag !== '' && preg_match("/^([a-zA-Z0-9_&\-\,\|]+)\$/i", $flag)) {
        ...
        // 如果有筛选标签,则采用子查询
        if ($tags) {
            $tagIds = Tag::where('name', 'in', explode(',', $tags))->cache(true, null, 'cms')->column("id");
        ...
        if ($paginate) {
            ...
            $list = $archivesModel->limit($limit)->cache($cacheKey, $cacheExpire, 'cms')->select();
        }

        //主表自定义字段额外属性
        Service::appendTextAndList('archives', 0, $list, true);
    ...
    public static function render(&$list, $imgwidth, $imgheight)
    {
        ...
        foreach ($list as $k => &$v) {
            $v['textlink'] = '<a href="' . $v['url'] . '">' . htmlentities($v['title']) . '</a>';
            $v['channellink'] = '<a href="' . ($v['channel']['url'] ?? '#') . '">' . htmlentities($v['channel']['name'] ?? '未知') . '</a>';
            $v['imglink'] = '<a href="' . $v['url'] . '"><img src="' . htmlentities($v['image']) . '" ' . $width . ' ' . $height . ' /></a>';
            $v['img'] = '<img src="' . htmlentities($v['image']) . '" ' . $width . ' ' . $height . ' />';
        }
        return $list;
    }
