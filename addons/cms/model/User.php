...
class User extends \app\common\model\User
{
    ...
    public static function getUserList($params)
    {
        ...
        if ($paginate) {
            ...
            $list = $userModel->limit($limit)->cache($cacheKey, $cacheExpire, 'cms')->select();
        }
    ...
    public static function render(&$list, $imgwidth, $imgheight)
    {
        ...
        foreach ($list as $k => &$v) {
            $v['textlink'] = '<a href="' . $v['url'] . '">' . htmlentities($v['nickname']) . '</a>';
            $v['imglink'] = '<a href="' . $v['url'] . '"><img src="' . htmlentities($v['avatar']) . '" ' . $width . ' ' . $height . ' /></a>';
            $v['img'] = '<img src="' . htmlentities($v['avatar']) . '" ' . $width . ' ' . $height . ' />';
        }
        return $list;
    }
