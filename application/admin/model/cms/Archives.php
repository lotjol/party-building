...
class Archives extends Model
{
    ...
    private function buildUrl($value, $data, $domain = false)
    {
        ...
        $vars = [
            ...
            ':diyname'  => $diyname,
            ':channel'  => $data['channel_id'] ?? $cateid,
            ':catename' => $catename,
            ':cateid'   => $cateid,
            ':year'     => date("Y", $time),
            ':month'    => date("m", $time),
    ...
    protected static function init()
    {
        ...
        self::beforeWrite(function ($row) {
            ...
            if (isset($changedData['publishtime']) && $changedData['publishtime'] > time() && $row['status'] == 'normal') {
                $row['status'] = 'prepare';
            }
        });
        self::afterWrite(function ($row) use ($config) {
            ...
            //刷新统计数据
            $refreshIds = array_merge([$row['channel_id'] ?? '', $changedData['channel_id'] ?? ''], explode(',', $row['channel_ids'] ?? ''), explode(',', $changedData['channel_ids'] ?? ''));
    ...
    {
        return ['normal' => __('Status Normal'), 'hidden' => __('Status Hidden'), 'draft' => __('Status Draft'), 'prepare' => __('Status Prepare'), 'rejected' => __('Status rejected'), 'pulloff' => __('Status pulloff')];
    }

    public function getCreatebyList()
    {
        return ['user' => __('Createby user'), 'api' => __('Createby api'), 'admin' => __('Createby admin')];
    }
