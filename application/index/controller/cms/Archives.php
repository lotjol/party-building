...
class Archives extends Frontend
{
    ...
    public function post()
    {
        ...
        if (!$archives && !$model) {
            $firstChannel = Channel::where('status', 'normal')->where(function ($query) use ($config) {
                // 如果频道封面不允许投稿则忽略
                if (!($config['channelcontribute'] ?? true)) {
                    $query->where('type', '<>', 'channel');
                }
            })->where('iscontribute', 1)->order('weigh DESC,id DESC')->find();
            if ($firstChannel) {
        ...
        $fields = \addons\cms\library\Service::getCustomFields('archives', 0, $archives ? $archives->toArray() : [], []);

        $this->view->assign('fields', $fields);
        $this->view->assign('values', $archives ? $archives->toArray() : []);
