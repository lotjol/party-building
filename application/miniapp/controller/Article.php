...
class Article extends Base
{
    ...
    public function detail()
    {
        ...
        try {
            ...
            // 获取文章内容
            $content = '';
            $articleData = \think\Db::name('cms_addon_article')->where('id', $id)->find();
            if ($articleData) {
     ...
     * @ApiParams (name="channel_id", type="int", required=false, description="栏目ID")
     * @ApiDescription 热门文章固定获取flag=hot的数据，按浏览量排序
     */
    public function hot()
    {
        $limit = $this->request->get('limit', 10);
        ...
        try {
            // 构建查询条件 - 只查询通用文章模型(model_id = 1)且flag=hot
            $where = [
                'status' => 'normal',
                'model_id' => 1,  // 通用文章模型ID
                'flag' => 'hot'   // 固定筛选flag=hot
            ];
    ...
    public function search()
    {
        ...
        try {
            ...
            // 获取总数
            $totalQuery = CmsArchives::where('status', 'normal')
                ->where('model_id', 1)  // 通用文章模型ID
                ->where('title', 'like', '%' . $keyword . '%');
            
            if ($channelId) {
                $totalQuery->where('channel_id', intval($channelId));
            }
            ...
            if ($flag) {
                $flags = array_filter(explode(',', $flag));
                if (!empty($flags)) {
                    $totalQuery->where('flag', 'in', $flags);
                }
            }
